...
namespace Framework.Pipeline.Standard.ThemeApplicator.Recipe
{
    ...
    public class PairedAreaRecipe : GameWorldObjectRecipe
    {
        ...
        {
            if (individual.GetShape() is OwPolygon shapeAsPolygon)
            {
                GameObject mesh;
                try
                {
                    mesh = GameObjectCreation.CombineMeshesFromPolygon(shapeAsPolygon, floorMaterial);
                }
                catch (Exception e)
                {
                    Console.WriteLine(e);
                ...
                Vector2 centroid = individual.GetShape().GetCentroid();
                    ...
                    if (i == 0)
                    {
                        ...
                        GameObject instantiated =
                            GameObjectCreation.InstantiatePrefab(
                                centerPieces[(int) (lRandom.NextDouble() * centerPieces.Length)],
                                vector2);
                        instantiated.transform.parent = mesh.transform;
                    }
                    else
                    {
                        if (lRandom.NextDouble() < 0.5)
                        {
                            ...
                            foreach (Vector2 flowerPatchPoint in flowerPatchPoints)
                            {
                                ...
                                GameObject instantiated =
                                    GameObjectCreation.InstantiatePrefab(
                                        vegetationPrefabs[(int) (lRandom.NextDouble() * vegetationPrefabs.Length)],
                                        flowerPatchPoint);
